...
logga(0, "Connesso a Telegram per invio alert di movimento")
...
if tempo_da_avvio > float(ConfigSectionMap("Sistema")['tempo_ingaggio']):
    messaggio = "Ehi! Ho rilevato un movimento!"
    bot.sendMessage(ConfigSectionMap("Sistema")['id_chat'], messaggio)
    logga(1, "Movimento rilevato e utenti avvisati")
else: 
    logga(1, "Movimento rilevato, utenti non avvisati perche' nel periodo di attivazione. Limite impostato: " + str(ConfigSectionMap("Sistema")['tempo_ingaggio']) + " Tempo passato: " + str(tempo_da_avvio))
